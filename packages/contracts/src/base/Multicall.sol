...
abstract contract Multicall {
    /// @notice Error when msg.value is sent to multicall (prevents double-spending via delegatecall)
    error MulticallNotPayable();
    ...
    function multicall(bytes[] calldata data) external payable returns (bytes[] memory results) {
        // C-01: Prevent msg.value double-spending across delegatecalls
        if (msg.value > 0) revert MulticallNotPayable();
